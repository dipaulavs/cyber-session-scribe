...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return <div className={`bg-cyber-darker border-r border-neon-blue/20 transition-all duration-300 flex flex-col ${sidebarExpanded ? 'w-72' : 'w-16'} relative z-10`}>
      {/* Sidebar Header */}
      <div className="p-4 border-b border-neon-blue/20 flex items-center justify-between">
        ...
        <button onClick={toggleSidebar} className="text-neon-blue hover:text-white p-1 rounded-md transition-colors duration-200 bg-cyber-button">
          {sidebarExpanded ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className={`${sidebarExpanded ? 'p-3' : 'p-2 flex justify-center'}`}>
        <button onClick={createNewSession} className={`${sidebarExpanded ? 'w-full bg-cyber-button text-neon-blue font-medium py-2 px-4 glow-text-blue' : 'bg-cyber-button text-neon-blue p-2 w-12 h-12 glow-text-blue'} rounded-md flex items-center justify-center transition-all duration-200 hover:scale-105`} title="Create new session">
      ...
      {/* Search Box */}
      {sidebarExpanded && <div className="px-3 pb-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search sessions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-cyber-input text-gray-300 w-full pl-10 pr-4 py-2 rounded-md border border-neon-blue/20 focus:outline-none focus:border-neon-blue/50 focus:glow-border transition-all duration-200" />
          </div>
        </div>}
      
      {/* Session List */}
      <div className="flex-1 overflow-y-auto cyber-scrollbar cyber-grid">
        {sessions.map(session => <div key={session.id} onClick={() => selectSession(session.id)} className={`p-4 border-b border-neon-blue/10 cursor-pointer transition-all duration-200 ${session.active ? 'bg-cyber-active' : 'hover:bg-cyber-button'}`}>
            <div className="flex items-center">
      ...
      {/* Sidebar Footer */}
      {sidebarExpanded && <div className="p-3 border-t border-neon-blue/20 text-xs text-gray-500">
